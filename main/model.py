...
class GP(gpytorch.models.ExactGP):
    def __init__(self, kernel, likelihood, points, Z0):
        super(GP, self).__init__(points, Z0, likelihood)
...
class data():
    def __init__(self, T, XY, Z, XYP_UV = None, XY_full = None, Z_full = None):
        ...
        self.output_std =  self.Z.reshape(-1).std()
        self.Z_normalized = [(z - self.output_mean)/self.output_std for z in self.Z]
        self.Z0_normalized =  self.Z_normalized[0]

        self.input_mean = self.XY.mean(dim=-2, keepdim=True)
        self.input_std = (self.XY - self.input_mean).max(dim = 0)[0][0] * torch.ones(2).unsqueeze(0)
        self.XY_normalized = (self.XY - self.input_mean)/self.input_std
        
        self.time_std = self.T.max()
        self.T_normalized = self.T/self.time_std
    ...
    def plot_vel_data(self,
        ...
        if indices is not None:
            ...
            XY_UV = XY_UV_
        return plot.velocities(XY_UV, self.extent, frame, color, gif)
