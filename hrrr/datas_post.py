...
for data in filtered_fit:
    row = [
        data.date,
        data.level,
        round(data.gp.kernel.outputscale.item() * data.output_std.item(), 4),
        round(data.gp.kernel.base_kernel.lengthscale[0][0].item(), 4),
        round(data.gp.kernel.base_kernel.lengthscale[0][1].item() * data.input_std.item(), 4),
        round(data.gp.kernel.base_kernel.lengthscale[0][2].item() * data.input_std.item(), 4),
        round(data.gp.likelihood.noise.item() * data.output_std.item(), 4),
        round(tools.rmse(data, mag=0).item(), 4),
        round(tools.rmse(data, scale=1/86400).item(), 4)
    ]
    table.append(row)
    ...
    for frame_num in frames:
        indices = torch.linspace(0,data.m -1, data.m).to(torch.int)
