...
class VectorField:
    def __init__(self):
        self.velocity = models.neural_flow.Velocity(width=32, depth=4)
    ...
    def train_gp(self, dvf, scalar_field, batch_size=1000, epochs=10):
        ...
        train.optim.mle(TXY_scaled, Z_scaled, self.gp, batch_size, epochs, self, dvf)
    ...
    def train_flow(self, dvf, scalar_field, batch_size=1000, epochs=10):
        ...
        upper_noise = tau2 + 0.0001
        lower_noise = tau2 - 0.0001
        
        upper_mean = mean + 0.0001
        lower_mean = mean - 0.0001
        
        upper_ls = ls + torch.tensor([0.0001, 0.0001, 0.0001]) 
        lower_ls = ls - torch.tensor([0.0001, 0.0001, 0.0001]) 
